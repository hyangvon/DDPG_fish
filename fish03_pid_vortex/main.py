...
##########  与 Vortex 有关的变量定义  ##########
V_1c = 0                                    # 第一关节质心与水流的合速度垂直于鱼体表面的线速度分量
V_1 = 0                                     # 第一关节质心与水流的合速度
V_1_v = np.array([0, 0])[np.newaxis, :]

V_2c = 0                                    # 第二关节质心与水流的合速度垂直于鱼尾表面的线速度分量
V_2 = 0                                     # 第二关节质心与水流的合速度
V_2_v = np.array([0,0])[np.newaxis, :]

X_1 = 0.32                                  # 第一关节质心横坐标
pos_c1 = np.array([0.32,0])[np.newaxis, :]
X_2 = 0.52                                  # 第二关节质心横坐标
pos_c2 = np.array([0.52,0])[np.newaxis, :]

# I_2_v = np.array([0, 0])[:, np.newaxis] # 鱼体 tail 单位法向量
...
##########  导入vortex数据  ##########
vortex_matrix = np.loadtxt(open("wave_position.csv", "rb"), delimiter=",", skiprows=0)
vortex_rows = vortex_matrix.shape[0]        # 返回行数
# print(vortex_matrix)
D1 = np.zeros((1, 18127))                   # 存放第一关节质心与涡街中心点距离
D2 = np.zeros((1, 18127))                   # 存放第二关节质心与涡街中心点距离
X1 = np.array([0, 0],dtype=float)[np.newaxis, :]    # 存放涡街速度数据
X2 = np.array([0, 0],dtype=float)[np.newaxis, :]    # 存放涡街速度数据
...
for t in np.arange(dt, time, dt):           # 从0.01秒到结束
    ...
    ##########  筛选要叠加的涡街  ##########
    for j in np.arange(0, vortex_rows, 1):  # 计算两关节质心与涡街点的距离
        D1[0, j] = math.pow((vortex_matrix[j, 0] - pos_c1[0, 0]), 2) + math.pow((vortex_matrix[j, 1] - pos_c1[0, 1]), 2)
        D2[0, j] = math.pow((vortex_matrix[j, 0] - pos_c2[0, 0]), 2) + math.pow((vortex_matrix[j, 1] - pos_c2[0, 1]), 2)
    m1 = np.argmin(D1)                      # 距离最小值（第一关节）对应序号
    m2 = np.argmin(D2)                      # 距离最小值（第二关节）对应序号
    X1[0, 0] = vortex_matrix[m1, 2]     # 选定质心最近点的涡街V_x,(第一关节) # 可以通过在此乘以某个倍数来改变涡街的强度
    X1[0, 1] = vortex_matrix[m1, 3]     # 选定质心最近点的涡街V_y,(第一关节)
    X2[0, 0] = vortex_matrix[m2, 2]     # 选定质心最近点的涡街V_x,(第二关节)
    X2[0, 1] = vortex_matrix[m2, 3]     # 选定质心最近点的涡街V_y,(第二关节)
    ...
    # 原速度叠加 vortex
    V_1_v = V_c1_v + X1
    V_1c = np.dot(V_1_v, I_1)
    V_1c_t.append(V_1c[0, 0])

    # 第一关节受力
    if V_1c[0,0] > 0:  # 当摆动方向和法向量方向相同时，身体部分水动力
        F_1_v = np.array([-0.5 * rou * C_1 * V_1c[0,0] ** 2 * A_1 * np.sin(theta_10),
                          0.5 * rou * C_1 * V_1c[0,0] ** 2 * A_1 * np.cos(theta_10)])[np.newaxis,:]
    ...
    # 原速度叠加 vortex
    V_2_v = V_c2_v + X2
    V_2c = np.dot(V_2_v, I_2)
    V_2c_t.append(V_2c[0, 0])

    # 第二关节受力
    if V_2c[0,0] > 0:  # 当摆动方向和法向量方向相同时，尾部水动力
        F_2_v = np.array([-0.5 * rou * C_2 * V_2c[0,0] ** 2 * A_2 * np.sin(theta_20),
                          0.5 * rou * C_2 * V_2c[0,0] ** 2 * A_2 * np.cos(theta_20)])[np.newaxis,:]
    ...
    # 两关节质点位置
    pos_c1 = pos_c1 + V_c1_v * dt  # 第一关节质点位置，矢量
    pos_c2 = pos_c2 + V_c2_v * dt  # 第二关节质点位置，矢量
    #print(pos_c1,pos_c2)
    ...
    # if (counter % 30) == 1:
    #     my_file = open('PID_eta_vortex.txt', 'a')
    #     text = 'ep_r: %.1f\n' % (100*total_eta)
    #     my_file.write(text)
    #     my_file.close()

    # 保存角度、角速度等变量数据到txt，用于后续绘图对比
    # my_file = open('PID_state_vortex.txt', 'a')
...
# plt.savefig('./Figure1_speed_vortex.pdf')
# plt.savefig('./Figure1_speed_vortex.png')
...
# # plt.ylim(-0.6, 0.6)
# plt.xlim(-100, 3100)
# ax1.plot(theta_10_t, linewidth=2, label='1st joint')   # 第一关节角度
...
# [label.set_fontname('Times New Roman') for label in labels]
# plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2, handlelength=1.0)
# plt.savefig('./Figure2_angle_vortex.pdf')
# plt.savefig('./Figure2_angle_vortex.png')
...
# # ax1.set_title('Angle Velocity of the fish', font_2)
...
# [label.set_fontname('Times New Roman') for label in labels]
# plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2, handlelength=1.0)
# plt.savefig('./Figure3_anglevelocity_vortex.pdf')
# plt.savefig('./Figure3_anglevelocity_vortex.png')
...
# plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2, handlelength=1.0)
# plt.tight_layout()
# plt.savefig('./Figure4_acceleration_vortex.pdf')
# plt.savefig('./Figure4_acceleration_vortex.png')
...
# [label.set_fontname('Times New Roman') for label in labels]
# plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2, handlelength=1.0)
# plt.savefig('./Figure5_torque_vortex.pdf')
# plt.savefig('./Figure5_torque_vortex.png')
...
# [label.set_fontname('Times New Roman') for label in labels]
# plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2, handlelength=1.0)
# plt.savefig('./Figure6_eta_vortex.pdf')
# plt.savefig('./Figure6_eta_vortex.png')
