...
class DDPG(object):
    ...
    def choose_action(self, s):
        ...
        action[0] = np.clip(np.random.normal(action[0], self.var1), 1, 21)#9, 31)  # 0.0089
        action[1] = np.clip(np.random.normal(action[1], self.var2), 39, 61)  # 0.028
        return action
    ...
    def _build_a(self, s, scope, trainable):
        with tf.variable_scope(scope):
            ## init_w = tf.contrib.layers.xavier_initializer()  # 返回一个初始化程序
