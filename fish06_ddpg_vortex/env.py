...
class FishEnv(object):
    ...
    def __init__(self):
        ...
        ## 关节质心位置
        self.X_1 = 0.32
        self.pos_c1 = np.array([0.32, 0])[np.newaxis, :]    # 第一关节质心横坐标
        self.X_2 = 0.52
        self.pos_c2 = np.array([0.52, 0])[np.newaxis, :]    # 第二关节质心横坐标
        ...
        ##=== 导入vortex数据 ===##
        self.vortex_matrix = np.loadtxt(open("wave_position.csv", "rb"), delimiter=",", skiprows=0)
        self.vortex_rows = self.vortex_matrix.shape[0]      # 返回行数
        # print(vortex_matrix)
        self.D1 = np.zeros((1, 18127))
        self.D2 = np.zeros((1, 18127))
        self.X1 = np.array([0, 0], dtype=float)[np.newaxis, :]
        self.X2 = np.array([0, 0], dtype=float)[np.newaxis, :]
    ...
    def step(self, action):
        ...
        ##=== vortex ===##
        for j in np.arange(0, self.vortex_rows, 1):
            # 计算关节中心点与涡街点的距离
            self.D1[0, j] = math.pow((self.vortex_matrix[j, 0] - self.pos_c1[0, 0]), 2) + math.pow(
                (self.vortex_matrix[j, 1] - self.pos_c1[0, 1]), 2)
            self.D2[0, j] = math.pow((self.vortex_matrix[j, 0] - self.pos_c2[0, 0]), 2) + math.pow(
                (self.vortex_matrix[j, 1] - self.pos_c2[0, 1]), 2)
        self.m1 = np.argmin(self.D1)  # 距离最小值（第一关节）对应序号
        self.m2 = np.argmin(self.D2)  # 距离最小值（第二关节）对应序号

        self.X1[0, 0] = 2 * self.vortex_matrix[self.m1, 2]  # 涡街V_x,(第一关节)
        self.X1[0, 1] = 2 * self.vortex_matrix[self.m1, 3]  # 涡街V_y,(第一关节)
        self.X2[0, 0] = 2 * self.vortex_matrix[self.m2, 2]  # 涡街V_x,(第二关节)
        self.X2[0, 1] = 2 * self.vortex_matrix[self.m2, 3]  # 涡街V_y,(第二关节)
        ...
        ##=== vortex ===##
        self.V_1_v = self.V_c1_v + self.X1
        self.V_1c = np.dot(self.V_1_v, self.I_1)
        self.V_1c_t.append(self.V_1c[0, 0])

        # 第一关节受力
        if self.V_c1c[0, 0] > 0:    # 当摆动方向和法向量方向相同时，身体部分产生的水动力
            self.F_1_v = np.array([-0.5 * self.rou * self.C_1 * self.V_1c[0, 0] ** 2 * self.A_1 * np.sin(self.theta_10),
                                   0.5 * self.rou * self.C_1 * self.V_1c[0, 0] ** 2 * self.A_1 * np.cos(self.theta_10)])[np.newaxis, :]
        ...
        ##=== vortex ===##
        self.V_2_v = self.V_c2_v + self.X2
        self.V_2c = np.dot(self.V_2_v, self.I_2)
        self.V_2c_t.append(self.V_2c[0, 0])

        # 第二关节受力
        if self.V_c2c[0, 0] > 0:    # 当摆动方向和法向量方向相同时，尾部产生的水动力
            self.F_2_v = np.array([-0.5 * self.rou * self.C_2 * self.V_2c[0, 0] ** 2 * self.A_2 * np.sin(self.theta_20),
                                   0.5 * self.rou * self.C_2 * self.V_2c[0, 0] ** 2 * self.A_2 * np.cos(self.theta_20)])[np.newaxis, :]
        ...
        ############### vortex #################
        self.pos_c1 = self.pos_c1 + self.V_c1_v * self.dt  # 鱼身体中心位置，矢量
        self.pos_c2 = self.pos_c2 + self.V_c2_v * self.dt  # 鱼身体中心位置，矢量
        #print(pos_c1,pos_c2)

        # 两关节角加速度、前进加速度
        self.theta_10_2o = g_acc[0, 0]                      # 第一关节角加速度
        self.theta_21_2o = g_acc[1, 0]                      # 第二关节相对角加速度
        self.X_0_2o = g_acc[2, 0]                           # 前进加速度
        ...
        self.U = self.X_0_1o
        self.theta_10_1o_t.append(self.theta_10_1o)
        ...
        self.theta_10_t.append(self.theta_10)
        ...
        ####################  数据保存（不要重复保存）  ####################
        my_file = open('DDPG_state_vortex.txt', 'a')
        ...
        if (self.counter % 5000) == 0:
            ...
            # fig = plt.figure(1, figsize=(15, 10))#, figsize=(15, 10)
            ...
            # plt.savefig('./Figure1_speed_vortex_2x.pdf')
            # plt.savefig('./Figure1_speed_vortex_2x.png')
            ...
            # plt.ylim(-0.6, 0.6)
            # plt.xlim(-100, 5100)
            # ax1.plot(self.theta_10_t, linewidth=2, label='1st joint')
            ...
            # plt.ylim(-0.6, 0.6)
            # plt.xlim(-100, 5100)
            # ax2.plot(self.theta_21_t, linewidth=2, label='2nd joint', color='orange')
            ...
            # [label.set_fontname('Times New Roman') for label in labels]
            # plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2, handlelength=1.0)
            # plt.savefig('./Figure2v_angle.pdf')
            # plt.savefig('./Figure2v_angle.png')

            # plt.figure(3, figsize=(15, 10))   # 绘制关节角速度图
            ...
            # plt.ylim(-4, 4)
            # plt.xlim(-100, 5100)
            # ax1.plot(self.theta_10_1o_t, linewidth=2, label='1st joint')
            # ax1.set_ylabel('angle velocity(rad/s)', font)
            # plt.tick_params(labelsize=40)
            # labels = ax1.get_xticklabels() + ax1.get_yticklabels()
            # [label.set_fontname('Times New Roman') for label in labels]
            # plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2,
            #            handlelength=1.0)
            #
            # ax2 = fig.add_subplot(212)
            # x = range(0, 5500, 500)
            ...
            # plt.ylim(-4, 4)
            # plt.xlim(-100, 5100)
            # ax2.plot(self.theta_21_1o_t, linewidth=2, label='2nd joint', color='orange')
            # ax2.set_xlabel('time(s)', font)
            # plt.subplots_adjust(left=0.12, right=0.9, top=0.9, bottom=0.13)
            # plt.tick_params(labelsize=40)
            # labels = ax2.get_xticklabels() + ax2.get_yticklabels()
            ...
            # plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2,
            #            handlelength=1.0)
            # plt.savefig('./Figure3v_anglevelocity.pdf')
            # plt.savefig('./Figure3v_anglevelocity.png')
            ...
            # plt.figure(5, figsize=(15, 10))   # 绘制关节转矩图
            ...
            # plt.ylim(-4, 4)
            # plt.xlim(-100, 5100)
            # ax1.plot(self.M_1_t, linewidth=2, label='1st joint')
            ...
            # plt.ylim(-1, 1)
            # plt.xlim(-100, 5100)
            # ax2.plot(self.M_2_t, linewidth=2, label='2nd joint', color='orange')
            # ax2.set_xlabel('time(s)', font)
            # plt.subplots_adjust(left=0.12, right=0.9, top=0.9, bottom=0.13)
            # plt.tick_params(labelsize=40)
            # labels = ax2.get_xticklabels() + ax2.get_yticklabels()
            ...
            # plt.legend(loc='lower right', prop=font_1, labelspacing=0.1, handletextpad=0.1, borderpad=0.2,
            #            handlelength=1.0)
            # plt.savefig('./Figure5v_torque.pdf')
            # plt.savefig('./Figure5v_torque.png')
