...
def train():
    ...
    for i in range(MAX_EPISODES):
        ...
        # plt.plot(eta, marker='o', linestyle='-', color='b', label='Data values')
        # plt.xlabel('Index')
        # plt.ylabel('Value')
...
def eval():
    ...
    for _ in range(1000):
        ...
        # print(ep_r)
    plt.figure(figsize=(10, 5))
    plt.plot(env.error, marker='o', linestyle='-', color='b', label='Data values')
    plt.xlabel('Index')
    plt.ylabel('Value')
    plt.title('Plot of List Values')
    plt.legend()
...
if ON_TRAIN:
    ...
    print(11)
    # 设置服务器地址和端口
    HOST = '0.0.0.0'  # 绑定到所有可用接口
    PORT = 6666       # TwinCAT 客户端连接的端口
    ...
    def read_state():
        state = conn.recv(24)
        if len(state) == 24:
            return np.frombuffer(state, dtype=np.float32)
        return None
